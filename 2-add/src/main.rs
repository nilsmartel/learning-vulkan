...
fn main() {
    ...
    let buffer_second =
        CpuAccessibleBuffer::from_iter(device.clone(), BufferUsage::all(), false, 0..1024)
            .expect("failed to create buffer");
    let buffer_result = CpuAccessibleBuffer::from_iter(
        device.clone(),
        BufferUsage::all(),
        false,
        (0..1024).map(|_| 0),
    ...
    let set = Arc::new(
        PersistentDescriptorSet::start(layout.clone())
            .add_buffer(buffer_first.clone())
            .unwrap()
            .add_buffer(buffer_second.clone())
            .unwrap()
            .add_buffer(buffer_result.clone())
            .unwrap()
            .build()
            .unwrap(),
    );
    ...
    let content = buffer_result.read().unwrap();
...
layout(set=0, binding=1) buffer Second {
    uint data[];
} second;

layout(set=0, binding=2) buffer Third {
    uint data[];
} result;

void main() {
    uint idx = gl_GlobalInvocationID.x;
    result.data[idx] = first.data[idx] + second.data[idx];
}
    "
    }
}
